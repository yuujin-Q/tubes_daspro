...
F.S. : user_inventory diupdate jika game berhasil dibeli atau tidak mengembalikan apa-apa jika game gagal dibeli
...
# REALISASI FUNGSI
def beli_game (user_inventory,user,game,userhistory,user_id):
        ...
        for i in range (1,common.iterLength(game)):   # cek game.csv
            if id_game == game[i][0] and int(game[i][5])>0:   # mencari id game dan jika stok game > 0
                if int(common.remove_thousands(game[i][4])) <= int(user[user_id+1][5]):   # jika saldo user cukup
                    ...
                    user_inventory += [[game[i][0],game[i][1],game[i][4],game[i][2],game[i][3]]]   # menambahkan data game ke user_inventory
                    userhistory += [[game[i][0],game[i][1],game[i][4],datetime.date.today().year]]   # menambahkan data riwayat ke userhistory
